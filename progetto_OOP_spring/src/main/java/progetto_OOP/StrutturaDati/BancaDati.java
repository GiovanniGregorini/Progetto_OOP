...
public class BancaDati {
	...
	/**
	 * Metodo ausiliario che conta le spese per le lauree effettuate
	 * @return contatore spese pel le lauree effettuate
	 */
	public int countspesa_laurea() {
		int contatore = 0;
		for(Dati elem : dati) {
			if (elem.getDati_spesa().getSpesa_laurea()>0)
			contatore++;
		}
		return contatore;
	}
	/**
	 * Metodo ausiliario che conta le spese per i dottorati effettuate
	 * @return contatore spese pel i dottorati effettuate
	 */
	public int countspesa_dottorato() {
		int contatore = 0;
		for(Dati elem : dati) {
			if (elem.getDati_spesa().getSpesa_dottorato()>0)
			contatore++;
		}
		return contatore;
	}
	/**
	 * Metodo ausiliario che conta le spese per le specializzazioni effettuate
	 * @return contatore spese pel le specializzazioni effettuate
	 */
	public int countspesa_specializzazione() {
		int contatore = 0;
		for(Dati elem : dati) {
			if (elem.getDati_spesa().getSpesa_specializzazione()>0)
			contatore++;
		}
		return contatore;
	}
